...
export const verifyDeliveryOtp = async (req, res) => {
  try {
    ...
    console.log("Verifying OTP:", { orderId, shopOrderId, otp }); // Debug Log

    const order = await Order.findById(orderId).populate("user");
    const shopOrder = order.shopOrders.id(shopOrderId);
    if (!order || !shopOrder) {
      console.log("Order or ShopOrder not found");
      return res.status(400).json({ message: "enter valid order/shopOrderid" });
    }
    
    console.log("Store OTP:", shopOrder.deliveryOtp);
    console.log("Expires:", shopOrder.otpExpires);
    console.log("Now:", new Date());

    // Normalize comparison
    if (
      String(shopOrder.deliveryOtp).trim() !== String(otp).trim() ||
      !shopOrder.otpExpires ||
      new Date(shopOrder.otpExpires) < Date.now()
    ) {
